...
for module_name in node_files:
    try:
        ...
        if hasattr(module, "NODE_CLASS_MAPPINGS") and hasattr(module, "NODE_DISPLAY_NAME_MAPPINGS"):
            ...
            # Логируем: выводим отображаемые имена всех нод из этого модуля
            ...
            if loaded_display_names:
                print(f"✅ Загружен модуль '{module_name}' → ноды: {', '.join(loaded_display_names)}")
            else:
                print(f"⚠️  Модуль '{module_name}' не содержит нодов (пустые маппинги)")

        else:
            print(f"⚠️  {module_name} не содержит NODE_CLASS_MAPPINGS или NODE_DISPLAY_NAME_MAPPINGS — пропущен")
